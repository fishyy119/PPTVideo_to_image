# PPTVideo_to_image（Python_branch）
## 运行环境

- Python版本：3.11.9
- 依赖项：见requirements.txt
...
其`requirements.txt` 同时包含了**OpenCV**库和**MoviePy**库，如果想要选择性配置，请注释掉`requirements.txt` 中的对应行。
...
6. 进度提示间隔
7. 相似度比较阈值
    > 此值为非负整数，此值越小，则对于图片相似度的判断越严格  
    > 当此项为0时，所有截取到的帧都会被输出，相似性判断失效  
    > 当此项为1时，相似度判断最为严格，
...
在**2帧**一跳的情况下，处理速度约为**OpenCV**版本的2.5倍。
